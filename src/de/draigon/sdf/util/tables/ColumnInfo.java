...
public class ColumnInfo {
        ...
        try {
            ...
            if (String.class.equals(clazzOfField)) {
                if (field.getField().isAnnotationPresent(DBFieldLength.class)) {
                    return ((DBFieldLength) field.getField().getAnnotation(DBFieldLength.class)).value();
                }else{
                    Loggin.logFieldWarn("no defined length for field '" + field.getName() +"' on type "+field.getMainObjectClass().getCanonicalName()+": taking default length (50).");
                    return 50;
                }
            }
        } catch (IllegalArgumentException e) {
    ...
    public boolean equals(Object obj) {
        ...
        if ("VARCHAR".equalsIgnoreCase(type)){
            if(length == null) {if (other.length != null)
                return false;
