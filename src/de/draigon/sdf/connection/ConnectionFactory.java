...
public class ConnectionFactory {
    ...
    /**
     * //Will be called, if factory is destroyed, so on system exit. 
     * Then in case of embadded database usage, the derby database is stopped.
     */
    @Override
    ...
    static {
        ...
        if (ConnectionFactory.embedded) {
            Loggin
                    .logConnectionFactory("using embedded database (org.apache.derby)");
        } else {
            Loggin
                    .logConnectionFactory("using MySQL database (default)");
        }
    ...
    /**
     * Returns a {@link DBConnection} from the pool. 
     * If no connection available, this method will wait to get one.
     * 
    ...
    /**
     * Determinates the connection pool to actual sizes (relative to default values).
     */
